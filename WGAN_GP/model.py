...
from GAN.WGAN_GP.networks import Critic, Generator, initialize_weights
from GAN.WGAN_GP.utils import save_graph, save_checkpoint, load_checkpoint

class WGAN_GP():
    def __init__(self, hyperparameter_set, load_model=True):
        with open("GAN/WGAN_GP/hyperparameters.yaml", "r") as file:
        ...
        self.critic = Critic(self.CHANNELS_IMG, self.FEATURES_CRITIC).to(self.device)
        ...
        self.runs_dir = "GAN/WGAN_GP/" + self.LOG_DIR
    ...
    def train(self):
        ...
        for epoch in range(self.epoch, self.NUM_EPOCHS + 1):
            for batch_idx, (real, _) in enumerate(self.loader):
                ...
                # Save losses
                if batch_idx % 5 == 0 and (batch_idx > 20 or self.load_model):
    ...
    def gradient_penalty(self, real, fake):
        ...
        mixed_scores = self.critic(interpolated)
